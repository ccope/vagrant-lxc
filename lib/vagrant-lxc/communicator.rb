...
module VagrantPlugins
  module LXC
    ...
    # actual LXC container via SSH.
    class Communicator < Vagrant.plugin("2", :communicator)
      def initialize(machine)
        @machine = machine
        @host_vm = machine.provider.host_vm

        # We only work on the LXC provider
        if machine.provider_name != :lxc
          raise Errors::CommunicatorNotLXC
        end
      end
      ...
      def download(from, to)
        ...
        # First, we use `cat` to copy that file from the LXC container.
        temp = "/tmp/lxc_d#{Time.now.to_i}_#{rand(100000)}"
      ...
      def execute(command, **opts, &block)
        ...
        remote_temp = "/tmp/lxc_#{Time.now.to_i}_#{rand(100000)}"
      ...
      def upload(from, to)
        ...
        to_temp = "/tmp/lxc_#{Time.now.to_i}_#{rand(100000)}"
        @host_vm.communicate.upload(from, to_temp)

        # Then, we use `cat` to get that file into the LXC container.
      ...
      def container_ssh_command
        ...
        # Use ad-hoc SSH options for the hop on the lxc proxy 
