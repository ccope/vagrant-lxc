...
module Vagrant
  module LXC
    module Action
      ...
      # we need to. The host machine is where LXC containers will
      # live.
      class HostMachineSyncFolders
        ...
        def initialize(app, env)
          @app    = app
          @logger = Log4r::Logger.new("vagrant::lxc::hostmachine")
        end
        ...
        def setup_synced_folders(host_machine, env)
          ...
          if existing_folders
            existing_folders.each do |impl, fs|
              fs.each do |_name, data|
                if data[:lxc_sfid] && data[:lxc_host_sfid] == host_sfid
                  existing_ids[data[:lxc_sfid]] = data
                end
              end
            end
          end
          ...
          our_folders.each do |type, folders|
            folders.each do |id, data|
              ...
              if type == :lxc
                # We don't use the LXC type explicitly on the host VM
                data.delete(:type)
              end
              ...
              data[:lxc_guestpath] = data[:guestpath]
              data[:lxc_sfid] = id
              data[:lxc_host_sfid] = host_sfid
              data[:id] = id[0...6] + rand(10000).to_s

              # If we specify exact then we know what we're doing
              if !data[:lxc__exact]
                data[:guestpath] =
                  "/var/lib/lxc/lxc_#{Time.now.to_i}_#{rand(100000)}"
              end
              ...
              if !existing_ids.key?(id)
                ...
                # TODO: Does VMware need something similar?
          ...
          if !new_config.synced_folders.empty?
            ...
            env[:machine].ui.output(I18n.t(
              "lxc.host_machine_syncing_folders"))
